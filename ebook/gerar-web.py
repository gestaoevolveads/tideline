...
.tl-modal{ position:fixed; inset:0; z-index:100; display:none; align-items:flex-end; justify-content:center;
  background:rgba(23,39,38,.55); backdrop-filter:blur(3px) }
.tl-modal.abre{ display:flex }
.tl-folha{ width:100%; max-width:440px; background:var(--paper); border-radius:20px 20px 0 0;
  padding:26px 22px calc(26px + env(safe-area-inset-bottom)); text-align:left;
...
.tl-folha p{ font-size:14.5px !important; color:var(--muted); line-height:1.6 !important; margin:0 0 16px }
.tl-folha ol{ margin:0 0 20px; padding-left:20px }
.tl-folha li{ font-size:14.5px !important; color:var(--deep); line-height:1.7 !important; margin-bottom:4px }
...
.tl-folha button{ flex:1; font:inherit; font-size:15px; font-weight:600; min-height:52px; padding:14px;
  border-radius:12px; cursor:pointer; border:1px solid rgba(23,39,38,.18); background:transparent; color:var(--muted) }
.tl-folha button.ok{ background:var(--deep); color:#fff; border-color:var(--deep) }
@media (prefers-reduced-motion:reduce){ .tl-folha{ animation:none } }
